...
calcul_temps_frequence <- function(x,y,f_min,f_max,method){
  if(method=='lsp'){
    require(lomb,quietly=TRUE)
    aaze  <- 1   
    f <- NaN
    result_lsp <- NaN
    # plot_lsp = T
    try(result_lsp <- lsp(y,times=x,from=f_min,to=f_max,plot=plot_lsp,ofac=100,alpha=0.1,normalize="press"),silent=TRUE)
    
    if(length(result_lsp)<=1) {
      diff_arr = NA
    }
    else {
      diff_res = diff(result_lsp$power) >=0
      diff_arr = NA
      if(length(diff_res) >=2) {
        for (i in 2:length(diff_res)) {
          if (diff_res[i] != diff_res[i - 1]) {
            diff_arr = append(diff_arr, result_lsp$power[i])
          }
        }
        
        diff_arr = diff_arr[2:length(diff_arr)]
      }
      # print(paste(length(diff_arr), abs(max(diff_arr)-min(diff_arr)), length(diff_arr)<6))
      # Sys.sleep(2)
    }
    ...
    if (plot_lsp) {
      ...
      if (length(diff_arr) < 8)
        try(
        ggsave(filename = tempfile(
          pattern = paste("_",length(diff_arr)),
          tmpdir = paste0("/tmp/lsp/true"),
          fileext = ".png"
        )), silent=T)
      else
        try(
        ggsave(filename = tempfile(
          tmpdir = paste0("/tmp/lsp/false"),
          fileext = ".png"
        )), silent=T)
    }
    return(f)
