...
proj_surf <- function(R_0,R_p,pos_xyz){ # Projection of the point (pos_xyz) on the ellipsoid (R_0,R_p)
  ...
  pos_geo <- pos2geo(R_0 = R_0, R_p = R_p,pos_xyz) # geographic coordinates of the considered point in the considered ellipsoid
  pos_proj_xyz <- geo2pos(R_0 = R_0, R_p = R_p, pos_geo = cbind(pos_geo[1],pos_geo[2],0)) # cartesian coordinates of the considered point projected on the considered ellipsoid (ellipsoid height set to 0)
